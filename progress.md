...
| 当前分支 | `dev` |
| 状态 | ✅ 已推送 |
| **版本标记** | **句号抑制机制重构：仿宋/楷体无条件抑制 + 思源宋体 U+3000 间隔抑制** |
| 标签 | `v1.4-font-suppress` |

### 本次会话变更（2026-07-31）

**1. 句号抑制机制重构 — 仿宋/楷体无条件抑制**

- **背景**：当前抑制机制要求句号前后两字在同一 CSR 内、同字体、中间有 U+3000 才抑制。仿宋/楷体区域的末尾句号（无后字）和跨 CSR 边界的句号无法被抑制。
- **新规则A（仿宋/楷体）**：
  - 前字是仿宋/楷体 → 无条件抑制（无论后字什么字体）
  - 后字是仿宋/楷体 且 前字不是 → 思源→仿宋过渡边界 → 放行（保留散文语法句号）
  - 段首 + 后字仿宋/楷体 → 抑制
- **规则B（思源宋体）**：维持现有逻辑不变（同CSR + 字间U+3000 → 抑制）
- **涉及文件**：`idml_utils.py`、`inject.py`（同步修改）
- **细节**：
  - `_SUPPRESS_PUNCT_FONTS` 拆分为 `_FONTS_SUPPRESS_WIDTH`（思源宋体，前缀匹配）+ `_FONTS_SUPPRESS_ALWAYS`（仿宋/楷体，包含匹配用 `in`）
  - `_is_suppress_target()` → `_is_suppress_width_target()` + `_is_suppress_always_target()`
  - `_should_suppress_punct()` 重写：规则A先行（非对称→仿宋侧抑制、思源侧放行），规则B在后
  - 新增 `_get_prev_non_ws_font()`、`_get_next_non_ws_font()` 支持前/后字体判定
- **测试**：11 项单元测试全部通过

**2. 批量匹配逻辑放宽**

- **改动**：`batch_inject.py` 的 `find_pairs()` 中 MD/TXT 识别从 `f.endswith("句读结果.md") or f.endswith("句读结果.txt")` 改为 `(f.endswith(".md") or f.endswith(".txt")) and "_WD注入" not in f`
- **效果**：不强制要求文件名包含"句读结果"，同经号 + 同扩展名即可配对；同时排除注入输出文件

### 历史会话（2026-07-28）

**1. 修复：IDML HTML 十六进制实体解码** — `fbc699e` / `0f05a00`

- **现象**：0080 文件注入失败——IDML 净文字数 5,644 vs 句读结果 2,929，差异在位置 3 处出现 `'x'` vs `'\U000fdc4f'`
- **根因**：0080.idml 正文 Story 中大量 Content 使用 `&#xNNNNN;` 格式的 HTML 十六进制实体编码表示增补平面字符（如 `&#xfdc4f;` → U+FDC4F）。解析器从 `<Content>` 标签提取文本后直接逐字符拆分，9 个 ASCII 实体字符全部被当作独立文字记录，导致字符数膨胀 2.2 倍
- **为何自检未发现**：`_verify_extraction()` 自检对原始 XML 和解析器结果做 1:1 比对，两边都未解码实体，所以"一致通过"——但两者都错了
- **为什么 175/275/3093 未受影响**：这些 IDML 由 InDesign 直接导出，文字以普通 UTF-8 存储；0080 是特殊来源的 IDML，其 Content 使用 InDesign XML 标准转义机制表示增补平面字符
- **修复**：
  - `idml_utils.py`：`_parse_paragraph_style_range()` 中 Content 提取后增加 `html.unescape()`；`_verify_parse_completeness()` 自检两处同步解码
  - `inject.py`：同步修复 4 处（解析 + 自检×2 + import html + import shutil）
- **验证**：ASCII 实体残留 0；增补平面字符 548 个正确解码；完整注入流水线通过（3,575 字符输出验证一致）
